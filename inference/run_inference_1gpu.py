...
def collate_fn(batch):
    ...
    correct_answers = [d[f"correct_answer"] for d in batch]
    input_prompts = [d[input_prompt] for d in batch]
    return inputs, identifiers, correct_answers, input_prompts

dataloader = DataLoader(dataset, batch_size=batch_size, collate_fn=collate_fn)

with torch.no_grad():

    for inputs, identifiers, correct_answers, input_prompts in tqdm(dataloader):
        ...
        for gen_text, prompt, identifier, correct_answer in zip(generated_texts, input_prompts, identifiers, correct_answers):
            all_results.append({
