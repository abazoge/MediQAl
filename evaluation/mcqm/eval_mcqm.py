...
list_files = [
	'../mcqm/8B/results_BioMistral-BioMistral-7B-DARE.json',
	'../mcqm/8B/results_deepseek-ai-DeepSeek-R1-Distill-Llama-8B.json',
]
...
for f in list_files:
	...
	results.append({
		"file_name": f,
		"emr_u": round(np.mean(results_emr_understanding), 4)*100,
		"emr_r": round(np.mean(results_emr_reasoning), 4)*100,
		"emr_avg": round(np.mean(results_emr_avg), 4)*100,
		"hamming_u": round(np.mean(results_hamming_understanding), 4)*100,
		"hamming_r": round(np.mean(results_hamming_reasoning), 4)*100,
		"hamming_avg": round(np.mean(results_hamming_avg), 4)*100,
	})


with open('result_mcqm_8B.json', 'w') as json_file:
